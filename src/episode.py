...
def load_episodes(file, episode, sol_per_pop):
    '''
    ...
    with open(file, "rb") as f:
        for _ in range(episode*sol_per_pop):
...
def generate_statistics(state_list, action_space, episodes):
    ...
    for s_from in s1:
        print("Processing state {}".format(s_from))
        for act in a:
            for s_to in s2:
                #convert to coord
...
def main():
    file_path = "/home/pal/Documents/RL/SimulationFramework/results/2020630192/trajectories_generation0.npy"
    episodes = load_episodes(file_path, episode=5, sol_per_pop=10)
    initial_state = (1, 1, 0)
    n_max_attempt = 6
    task_length = 11
