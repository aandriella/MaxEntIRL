...
def value_iteration(P_a, rewards, gamma, error=0.01, deterministic=True):
	...
	inputs:
	  P_a         NxN_ACTIONSxN transition probabilities matrix -
	...
	#using this value to avoid nan (0/0)
	...
	while True:
		...
		for s in range(N_STATES):
			v_s = []
			values[s] = max(
				[sum([P_a[s, s1, a] * (rewards[s] + gamma * values_tmp[s1]) for s1 in range(N_STATES)]) for a in range(N_ACTIONS)])
	...
	if deterministic:
		...
		for s in range(N_STATES):
			v_s = np.array([sum([P_a[s, s1, a] * (rewards[s] + gamma * values[s1]) for s1 in range(N_STATES)]) for a in
			                range(N_ACTIONS)])
			policy[s, :] = np.transpose(v_s / (np.sum(v_s)+eps))
		return values, policy
		...
		for i in range(iterations):
			...
			for s in states:
				...
				values[s] = sum([policy[s][i][1] * (self.mdp.get_reward(s) + self.gamma * sum([s1_p * values_tmp[s1]
				                                                                               for s1, s1_p in
				                                                                               self.mdp.get_transition_states_and_probs(
					                                                                               s, actions[i])]))
